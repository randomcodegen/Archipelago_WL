    ...
    async def game_watcher(self, ctx: BizHawkClientContext) -> None:
        ...
        if ctx.connected and ctx.data_present:    
            try:
                ...
                read_result = await bizhawk.guarded_read(ctx.bizhawk_ctx,
                                                         [(0xA000, 4, "System Bus")],
                ...
                if len(ctx.trap_queue)>ctx.traps_activated:
                    if ctx.demo_mode==0 and ctx.game_mode==3 and ctx.paused==0 and time.time()-ctx.last_trap_time>5:
                        # Grab a trap
                ...
                if ctx.event_update:
                    ...
                    # In Overworld
                        ...
                        await ctx.send_msgs([{
                            ...
                            "default": 0,
                            "want_reply": False,
                            "operations": [{"operation": "replace", "value":ctx.ow_id}]
                        }])
                    ctx.event_update=False
            except bizhawk.RequestFailedError:
